...
class _StaggedAnimationState extends State<StaggedAnimation>
  ...
  createMarkOnMap(type, _warningData) async {
    ...
    if (_warningData != null && _warningData.length > 0) {
      //显示图层
      _warningData.map((e) {
        var options = type == 'illegal'
            ? createIlleagalMarkerOption(e, type)
  ...
  createIlleagalMarkerOption(e, type) {
    if (e['longLat'] != null && e['longLat'].startsWith('point') == true) {
      ...
      if (double.parse(e['longLatTemp'][1]) >= -90.0 &&
          ...
          double.parse(e['longLatTemp'][0]) <= 180.0) {
        var option = MarkerOption(
            object: e['id'] + '_warning' + '_illegal' + '_marker',
            title: e['violationBeha'],
            snippet: type == 'illegal' ? e['violationBeha'] : e['id'],
            latLng: LatLng(double.parse(e['longLatTemp'][1]),
                double.parse(e['longLatTemp'][0])),
            infoWindowEnabled: false,
            visible: true,
  ...
  handleLayersClick(type) {
    ...
    //检测
      ...
      //预警信息图层最多打开一个图层
      getIllegalWarning(type, false);
    } else if (type == "yd") {
      // showNotification('yd', '拥堵预警');
      //预警信息图层最多打开一个图层
      getYDWarning(type, false);
    }
  }

  //车辆违法预警 检测
  getIllegalWarning(type, timerFlag) async {
    //判断从别的图层进入此图层 还是原本就是此图层
    if (!timerFlag) {
      //预警信息图层最多打开一个图层
      _warningLayers[type]['isActive'] = !_warningLayers[type]['isActive'];
      if (_warningLayers[type]['isActive'] == false) {
        await mapController.clear();
        _warningDataCache = new Map();
        _warningLayerCache = new Map();
        _warningMarkerCache = new Map();
        _warningDealingCache = new Map();
        _warningDealingMarkerCache = new Map();
        setState(() {
          _warningLayers;
        });
        return;
    ...
    _warningLayers.forEach((key, value) async {
      if (key == 'yd') {
        await mapController.showTraffic(false);
      }
      if (type != key) {
    ...
    //未处理事故警情
    String url =
        '/ControlPlatform/service/viorecord/findTodayVioList?currentOrgPrivilegeCode=' +
            userInfo[4] +
            '&vioType=12,26,27,31,14,other';
    var resPolice = await RequestApi.getIllegalWaring(url, !timerFlag);
    if (!timerFlag) {
      Navigator.of(context).pop();
    }
    if (resPolice?.data != null) {
      List temp = resPolice.data;
      if (resPolice.data.length > 4) {
        temp = resPolice.data.sublist(0, 4);
      }
      showNotification('illegal', '车辆违法预警');
      // //数据处理
      temp.forEach((value) {
        value['longLat'] = value['lonLat'];
      });
      //清空地图所有marker
      await mapController.clear();
      _warningDataCache = new Map();
      ...
      _warningDealingCache = new Map();
      _warningDealingMarkerCache = new Map();
      _warningDataCache['illegal'] = temp;
      createMarkOnMap('illegal', _warningDataCache['illegal']);
  ...
  //拥堵预警 检测
  getYDWarning(type, timerFlag) async {
    //判断从别的图层进入此图层 还是原本就是此图层
    if (!timerFlag) {
      //预警信息图层最多打开一个图层
      _warningLayers[type]['isActive'] = !_warningLayers[type]['isActive'];
      _warningLayers[type]['isActive'] == true
          ? await mapController.showTraffic(true)
          : await mapController.showTraffic(false);
    }

    _warningLayers.forEach((key, value) {
      if (type != key) {
        value['isActive'] = false;
      }
    });
  ...
  handleLayersShow(String type, bool timerFlag) async {
    ...
    if (!timerFlag) {
      ...
      _warningLayers[type]['isActive'] = !_warningLayers[type]['isActive'];
      //检测使用
      if (_warningLayers['yd'] != null && _warningLayers['yd']['isActive']) {
        _warningLayers['yd']['isActive'] = false;
        await mapController.showTraffic(false);
      }
      if (_warningLayers['illegal'] != null &&
          _warningLayers['illegal']['isActive']) {
        _warningLayers['illegal']['isActive'] = false;
        //清空地图所有marker
        await mapController.clear();
        _warningDataCache = new Map();
        _warningLayerCache = new Map();
