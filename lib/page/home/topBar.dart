...
class _StaggedAnimationState extends State<StaggedAnimation>
  ...
  getPolicingWarning(type, timeFlag) async {
    ...
    if ((!_warningLayers[type]['isActive'] && timeFlag) ||
      ...
      if (resNotification != null) {
        List temp = resNotification.data['result']['rows'];
        _warningDataCache['policing'] = new List();
        temp.forEach((element) {
          if (['1', '3', '16'].contains(element['eventType']))
            _warningDataCache['policing'].add(element);
        });
        if (_warningDataCache['policing'].length > 0)
    ...
    if (resPolice != null) {
      ...
      List temp = resPolice.data['result']['rows'];
      _warningDataCache['policing'] = new List();
      temp.forEach((element) {
        if (['1', '3', '16'].contains(element['eventType']))
          _warningDataCache['policing'].add(element);
  ...
  getPolicingData(timeFlag) async {
    ...
    //         userInfo[4] +
    //         '&handleState=0&eventType=16&pageNumber=1&pageSize=50';

    //时间
    DateTime now = DateTime.now();
    var formatterTime = DateFormat('yyyy-MM-dd HH:mm:ss');
    var startTime = formatterTime.format(now.subtract(new Duration(hours: 3)));
  ...
  Map getDetaiInfo(type, idName, id) {
    ...
    for (int i = 0; i < _warningDataCache[type].length; i++) {
      if (_warningDataCache[type][i][idName] == id)
        detail = _warningDataCache[type][i];
    }
    return detail;
  }
      ...
      await showDialog(
          ...
          builder: (BuildContext context) {
            return StatefulBuilder(builder: (context, _state) {
              ...
              String url = '';
              String _type = info[2];
              String idName = info[2] == 'meteo' ? 'alarmId' : 'eventId';
              detail = getDetaiInfo(_type, idName, info[0]);
              // Map<String, String> headersMap = {'Authorization': TOKEN};
              if (_type == 'meteo') {
                //获取气象信息
